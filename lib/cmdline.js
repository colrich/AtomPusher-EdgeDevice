...
module.exports = function(description, args, processFunction, argumentHelp) {
   var doHelp = function() {
	 ...
	 '  -u, --url=URL                    set URL of feed to republish\n' +
	 '  -t, --tag=TAG                    set TAG of this feed (feed tag used in later processing; choose unique value)\n' +
   ...
   args = minimist(args, {
      string: ['certificate-dir', 'aws-region', 'private-key', 'client-certificate',
         'ca-certificate', 'client-id', 'thing-name', 'configuration-file',
	       'host-name', 'protocol', 'url', 'tag'
      ],
      integer: ['reconnect-period-ms', 'port', 'delay-ms',
         'keepalive'
      ],
      boolean: ['help', 'debug'],
      alias: {
